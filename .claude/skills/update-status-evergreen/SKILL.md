...
## 1. Sync PR state and close rationale from GitHub

For every bug and security alert that has a `pr_url`, fetch the PR's state and comments:

```bash
gh pr view <pr_url> --json state,comments
```

Update `pr_status` in the database to match the state (`open`, `merged`, `closed`). When the PR is closed, read the owner's comments — ignore automated review-bot noise (e.g. Greptile) — and carry that rationale into §2.

## 2. Update issue status based on PR state

- **PR merged**: Mark the issue `resolved` and set `resolved_at` to now.
- **PR closed (not merged)**: the fix didn't land — which on its own says nothing about whether the bug is real. Judge from *this PR's own close comment only*, never another issue's. With no owner comment, send a verified bug to `backlog` and a never-verified one to `new`. With an owner comment:
  - not a real bug, or not worth fixing → `dismissed` (`--dismissed-by owner`)
  - the owner asked you to build a specific different fix → `action_needed`; record that fix in `--disposition-reason`
  - real, but no specific fix was requested → `backlog`

  Always record the owner's reason in `--disposition-reason` and keep the PR link.
- **PR open**: Leave as `in_progress`. Optionally check for review comments or CI failures and note them in the summary.

## 3. Check for Discord replies
...
- If the reply says to close, ignore, or dismiss, mark the issue `dismissed` with `--dismissed-by owner` and the owner's reason in `--disposition-reason`.
